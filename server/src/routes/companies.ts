...
export function companyRoutes(db: Db, storageService?: StorageService) {
  ...
  // Logo assets are served without board auth — they are referenced by <img> tags
  // which cannot include Authorization headers. URLs contain unguessable object keys.
  ...
  // POST /companies/:companyId/logo — upload company logo
  router.post("/:companyId/logo", async (req, res, next) => {
    assertBoard(req);
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    ...
    // Check company exists before uploading (Finding 5)
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        upload.single("file")(req, res as never, (err: unknown) => {
          if (err) reject(err);
          else resolve();
        });
      });
